// services/gemini.service.js - Módulo de Inteligencia Artificial
...
async function llamarGeminiConReintentos(apiKey, payload) {
    ...
    for (let intento = 0; intento < MAX_REINTENTOS; intento++) {
        if (intento > 0) {
            ...
            console.warn(`[Motor IA] Servidor ocupado. Reintento ${intento}/${MAX_REINTENTOS - 1} en ${espera}ms...`);
...
async function analizarComida(apiKey, imagenes, dieta, notas, caloriasObjetivo, bmr, nombreAtleta) {
    const gastoTotal = (bmr || 0) + caloriasObjetivo;
    ...
    - Metabolismo Basal (BMR): ${bmr || 'Desconocido'} kcal/día.
    - Gasto calórico activo promedio: ${caloriasObjetivo} kcal/día.
    - GASTO ENERGÉTICO TOTAL DIARIO (TDEE): ${gastoTotal} kcal/día.
    - Notas del atleta: "${notas || 'Sin notas adicionales.'}"

    === MISIÓN ===
    Analiza las imágenes adjuntas que componen el servicio completo de comida del atleta.
    Estructura tu respuesta exactamente con estas etiquetas integrando todos los alimentos detectados:
    [KCAL_ESTIMADAS] -> (Solo el número aproximado de kcal totales sumando todos los platos).
    [MACROS] -> (Detalla gramos estimados totales de Carbohidratos, Proteínas y Grasas del conjunto).
    [EVALUACION_OBJETIVA] -> (Crítica fisiológica estricta: ¿La suma de los platos cumple con la dieta ${dieta} y es adecuada considerando que el atleta requiere un TOTAL de ${gastoTotal} kcal hoy?).
    [RECOMENDACION_MEJORA] -> (Instrucción directa sobre si el servicio total ha sido excesivo, insuficiente o si fallaron las proporciones).`;
    ...
    try {
        ...
        const textoIA = await llamarGeminiConReintentos(apiKey, payload);

        if (!textoIA) throw new Error('Respuesta vacía de la IA');
        ...
        console.error('[Motor IA - Visión] Fallo al procesar el escáner múltiple:', error.message);
        return {
            kcal: 'Error', macros: 'Error', evaluacion: 'Los servidores de IA están saturados (Error 503).', recomendacion: 'Espera unos segundos y vuelve a pulsar el botón.'
        };
    }
}
